...
#        v = K.get_variable('v', shape=(ys_pred.shape[-1], 1))
        ...
        for i in range(self.nb_outputs+4):
...
#    plt.show() 
    ...
    plt.title('BRCA Weight Curves Without Penalty Term')
    plt.ylabel('Training-weight Value')
    plt.xlabel('epoch')
    plt.legend(['lambda1','lambda2','lambda3','lambda4'], loc='upper right')
    plt.savefig('Without Penalty Term MT_DSCCA lambda1-lambda4 Curves term315.jpg', dpi=300) #指定分辨率保存
    plt.show()
    
    plt.plot(range(EPOCH), hist.history['loss'])
    plt.title('BRCA Training Loss Curves Using MT_DSCCA')
    plt.ylabel('training-loss Value')
    plt.xlabel('epoch')
#    plt.legend(['${lambda0}$','${lambda1}$'], loc='upper right')
    plt.savefig('brca Loss Curves without penalty term315.jpg', dpi=300) #指定分辨率保存
    plt.show()
    
    
    np.savetxt("BRCA_methylation_mRNA_without_penalty_term_ml_ordCOX_20210106_2.csv",hist.history['loss'], delimiter=",")
    aaa=0
