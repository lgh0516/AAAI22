...
#    plt.show() 
    ...
    plt.title('BRCA Weight Curves With Penalty Term')
    plt.ylabel('Training-weight Value')
    plt.xlabel('epoch')
    plt.legend(['lambda1','lambda2','lambda3','lambda4'], loc='upper right')
    plt.savefig('With Penalty TermMT_DSCCA lambda1-lambda4 Curves term315.jpg', dpi=300) #指定分辨率保存
    plt.show()
...
#    plt.legend(['${lambda0}$','${lambda1}$'], loc='upper right')
    plt.savefig('brca Loss Curves with penalty term315.jpg', dpi=300) #指定分辨率保存
    plt.show()
    
    
    np.savetxt("all_with_penalty_BRCA_methylation_mRNA_MT_DSCCA_20210902.csv",hist.history['loss'], delimiter=",")
    aaa=0
#    import pylab
