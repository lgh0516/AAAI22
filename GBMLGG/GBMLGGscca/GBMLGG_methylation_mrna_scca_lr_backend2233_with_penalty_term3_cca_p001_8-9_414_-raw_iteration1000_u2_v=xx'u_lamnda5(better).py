...
#    plt.show() 
    ...
    plt.title('GBMLGG Weight Curves')
    plt.ylabel('Training-weight Value')
    plt.xlabel('epoch')
    plt.legend(['lambda1','lambda2','lambda3','lambda4'], loc='upper right')
    plt.savefig('GBMLGG MT_ADSCCA lambda1-lambda4 Curves term315.jpg', dpi=300) #指定分辨率保存
    plt.show()
    
    plt.plot(range(EPOCH), hist.history['loss'])
    plt.title('GBMLGG Training Loss Curves Using MT_ADSCCA')
...
#    plt.legend(['${lambda0}$','${lambda1}$'], loc='upper right')
    ...
    np.savetxt("GBMLGG_methylation_mRNA_MT_DSCCA_20210902.csv",hist.history['loss'], delimiter=",")
