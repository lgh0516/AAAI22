...
loss500_ordCOX1 = np.asarray(pd.read_csv("without_GBMLGG_methylation_mRNA_MT_DSCCA_20210902.csv",header=None))  
loss500_M_m = np.asarray(pd.read_csv("with_GBMLGG_methylation_mRNA_MT_DSCCA_20210902.csv",header=None))
...
plt.title('Loss Curves on GBMLGG (With/Without Penalty Term)')
plt.ylabel('training-loss')
plt.xlabel('epoch')
plt.legend(['The method without penalty term','The method with penalty term'], loc='upper right')
plt.savefig('GBMLGG_Loss Curves2_with_without penalty term.jpg', dpi=300) #指定分辨率保存
plt.show()
