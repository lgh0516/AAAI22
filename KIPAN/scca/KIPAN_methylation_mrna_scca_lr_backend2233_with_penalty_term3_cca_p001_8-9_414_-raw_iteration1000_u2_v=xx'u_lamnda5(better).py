...
#    plt.show() 
    ...
    plt.title('KIPAN Weight Curves')
    plt.ylabel('Training-weight Value')
    plt.xlabel('epoch')
    plt.legend(['lambda1','lambda2','lambda3','lambda4'], loc='upper right')
    plt.savefig('KIPAN MT_DSCCA lambda1-lambda4 Curves term315.jpg', dpi=300) #指定分辨率保存
    plt.show()
    
    plt.plot(range(EPOCH), hist.history['loss'])
    plt.title('KIPAN Training Loss Curves Using MT_DSCCA')
    plt.ylabel('train-loss Value')
    plt.xlabel('epoch')
#    plt.legend(['${lambda0}$','${lambda1}$'], loc='upper right')
    plt.savefig('KIPAN_Loss Curves without penalty term315.jpg', dpi=300) #指定分辨率保存
    plt.show()
    
    
    np.savetxt("KIPAN_methylation_mRNA_MT_DSCCA_20210902.csv",hist.history['loss'], delimiter=",")
